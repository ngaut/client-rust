...
| RPC context and routing | `RegionStore` separates logical peer/store from physical destination/proxy, retains endpoint/access indexes and location, replica/stale identity, bucket version, token ownership, forwarding host, store epoch, and health state across plan clones. `RpcContext` derives from that executable route and preserves full source strings, logical addresses, physical proxies, request/run store types, and endpoint-specific access indexes. Leader/follower/mixed/learner/TiFlash/all-store paths consume the authoritative cache snapshot. TiFlash routes expose source reason/store/peer diagnostics, skip stale store epochs, rotate on send failure, and trigger reload after exhaustion. |
...
| Test identity and sender diagnostics correction | A repository-wide scan after the first closeout found six `src/region_request.rs` identities still generated by a forwarding macro that the earlier file-scoped scan omitted. The macro is removed. `TestRegionRequestSenderString`, `TestRegionRequestStats`, `TestGetErrMsg`, `TestRPCContextString`, and `TestBackoffErrWithRPCContext` now execute their own source behavior directly; `TestLogging` sends a real Get through the retry plan and observes the hintless-NotLeader result. The stronger context ports also added production `RpcContext` derivation, complete protobuf text, endpoint-specific access indexes, proxy details, deepest transport-cause extraction, and contextual TiKV/TiFlash retry reasons. The corrected repository-wide gate reports 142 unique identities, zero differences, zero duplicates, zero forwarding mappings, and zero registered test-to-test calls. |
...
- Canonical `make unit-test`: 1,326 no-default workspace tests passed with two configured skips; 1,300 all-feature workspace-library tests passed with six configured skips. The two-test reduction removes obsolete aggregate helpers; all 142 source identities remain independently executable.
