...
| reusable crate boundary | `unistore` is one standalone crate and the normal dependency direction is `tikv-client -> unistore`; protocol conversion stays in `tikv-client`, avoiding a cycle and allowing other modules to reuse the engine. This package receipt covers only the client-go mocktikv behavior hosted there. TiDB's much larger UniStore package retains its independent `seed` row. |
