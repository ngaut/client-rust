...
| `txnkv/rangetask` | range plans / region iteration | unassessed | Requires region routing first. |
...
`internal/locate` sender seed: `RaftEntryTooLarge` now crosses the sender as a direct terminal error rather than a retryable Rust `RegionError`, preserving client-go's boundary and preventing RawKV's outer region-error retry from resubmitting the write. `UndeterminedResult` remains a caller-visible region payload. Validation: focused `region_error_actions_preserve_client_go_retry_classes` (1 passed), formatting, diff checks, and the full pinned-nightly library suite (367 passed) pass. The atomic source package receipt remains incomplete.
