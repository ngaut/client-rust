...
| `txnkv/rangetask` | `src/transaction/client.rs`, future native range-task runner | seed | Atomic source inventory: `range_task.go` (358 lines) and `delete_range.go` (185 lines); no package-local Go test, generated input, fixture, or build variant. Rust transactional `delete_range` now owns source `deleteRangeOneRegionMaxBackoff` (100,000-ms input, weighted by the same retry variables) instead of the generic region budget. A full receipt still requires caller-selected task concurrency, 128-region task batching, cancellation/error fan-out, stats/logging, notify-only DeleteRange, and native integration. |
