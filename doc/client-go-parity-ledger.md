...
| `internal/locate` | `src/region.rs`, `src/region_cache.rs`, `src/pd`, `src/request`, `src/store`, `src/traffic.rs` | in-progress | Atomic source inventory: production `accessmode.go` (62), `metrics_collector.go` (136), `pd_codec.go` (219), `region_cache.go` (3,472), `region_request.go` (2,449), `replica_selector.go` (744), `slow_score.go` (192), `sorted_btree.go` (151), and `store_cache.go` (1,222); support/tests `main_test.go` (32), `metrics_collector_test.go` (157), `pd_codec_test.go` (40), `region_cache_test.go` (3,497), `region_request3_test.go` (1,800), `region_request_state_test.go` (769), `region_request_test.go` (1,274), and `replica_selector_test.go` (3,916). No package-local generated/build/fixture/doc artifacts. Rust has only compatible seeds, including end-inclusive reverse lookup through PD `GetPrevRegion`, source exhausted-leader fallback to a follower leader-read probe, and partial sender/cache behavior. A real PdRpcClient transport failure now identifies the physical store independently of the logical peer, probes `grpc.health.v1.Health/Check` under the source `store-liveness-timeout`, records serving/unknown/unreachable status, and retains its region snapshot for selector-driven retry. Cached regions capture each peer store's failure epoch; an unhealthy physical store advances that epoch only if the request owns the captured snapshot, marks its health slow, and excludes stale replicas from retry selection. An unhealthy TiKV store owns one one-second health loop, which re-resolves its PD store metadata every 30 seconds while retaining the prior liveness state across refresh, and ends on recovery or client close. Configured `StoreLimit` now installs at `PdRpcClient` construction; it shares one deliberately optimistic atomic counter per logical region-route store, records client-go's address/store-labeled token-limit metric, and a Dispatch-lifetime guard releases a token after every transport result. The separate all-store route deliberately remains unlimited, as in client-go. Region-cache GC now runs from `PdRpcClient` construction until close at the source one-second cadence, scans at most 50 start keys per retained-cursor round, removes expired entries coherently from all three indexes, and marks live regions for expiration when a captured store epoch or liveness changes. All four source synchronization flags are represented: immediate reload, TTL freeze, delayed-pending, and delayed-ready; GC advances delayed reloads, lookups preserve old metadata and re-arm immediate reload on PD failure, down peers freeze TTL, stale eligible replicas schedule delayed reload, and exhausted forwarding schedules reload-on-access. Physical dispatch now implements the source network collector's exact request/response size matrices, KV-versus-MPP and local-versus-cross-zone atomic traffic totals, stale-read request/byte counters, and leader/follower local/remote read-byte observations; task-scoped details survive shard/retry spawning, and direct single-region plans retain the same complete route metadata as sharded plans. PD region coding now sits below `RegionCache` in a source-shaped wrapper: V1 raw/txn and numeric V2 modes encode point, previous, scan, batch-scan, and split inputs; decode region/bucket outputs; canonicalize keyspace metadata loads; reject disabled/V3 numeric-ID lookups; and retain the exact V2 keyspace-end scan bound. RawKV and transaction constructors build that wrapper before cache construction and reuse the loaded metadata, fixing the former prefixless V2 PD path. The store-cache lifecycle, access/health/slow-score, liveness, discovery, flow, and metric-cleanup slice is source-mapped and integrated. Selector state now preserves ten-attempt/fifty-second leader exhaustion, all source error flags, deferred per-store fast-retry backoff, logical/proxy RPC duration, cancellation without cache damage, one-attempt proxy rotation, and successful-proxy reuse. Remaining region-request/cache/index behavior and every still-unmapped original test remain required for one atomic receipt. |
