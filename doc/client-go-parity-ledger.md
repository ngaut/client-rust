...
| `txnkv/rangetask` | `src/transaction/range_task.rs`, `src/transaction/client.rs`, `src/transaction/requests.rs` | seed | Atomic source inventory: `range_task.go` (358 lines) and `delete_range.go` (185 lines); no package-local Go test, generated input, fixture, or build variant. Rust exposes caller-selected DeleteRange concurrency and `notify_only` execution, while the compatibility API defaults to one worker. Its runner covers ordered physical-region rediscovery, default 128-region groups, bounded concurrent handlers, completed/failed counters, shared first-error cancellation, a fresh source `deleteRangeOneRegionMaxBackoff` (100,000-ms input, weighted by the same retry variables) for every sent region, and source-exact terminal response-error handling. A full receipt still requires source metrics/progress logging, exact batch-load/locate retry behavior, and every source behavior. |
