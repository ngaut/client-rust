...
| `internal/locate` | `src/error.rs`, `src/region.rs`, `src/region_cache.rs`, `src/region_request.rs`, `src/pd`, `src/request`, `src/store`, `src/traffic.rs` | complete | Independently re-audited complete receipt below and full 17-artifact/production-symbol/147-declaration/dependency/consumer mapping in [`internal-locate-source-artifact-audit.md`](internal-locate-source-artifact-audit.md). A follow-up repository-wide scan corrected six `region_request` identities that the earlier file-scoped scan had missed: all 142 assertion-bearing Go declarations now have independently selectable ordinary Rust tests with no forwarding macros or test-to-test calls; `TestMain` and the four testify suite runners have explicit dispositions. The stronger sender tests fixed read-TS error erasure and restored complete RPC-context/backoff diagnostics, including endpoint-specific access indexes, proxies, full gogo protobuf text, and an actual hintless-NotLeader request path. Exact Go normal/race suites and both complete Rust configurations pass. Region coding and indexes, TTL/GC/refresh, store discovery/liveness/health, replica and proxy selection, sender retry/error precedence, runtime diagnostics, network accounting, configurable PD region-meta circuit breaking, and native async ownership are accounted for. Raw, snapshot, transaction, root `tikv`, and other consumers retain independent statuses. |
