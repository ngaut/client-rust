...
| `internal/mockstore/mocktikv` | hidden `src/mock/mocktikv`, reusable `unistore` engine crate, mock stream adapters | complete | Atomic receipt: [`internal-mockstore-mocktikv-source-artifact-audit.md`](internal-mockstore-mocktikv-source-artifact-audit.md). All 14 artifacts/6,689 lines, 23 ordinary tests plus goleak harness, complete MVCC/raw/pessimistic/debug/cluster/session/PD/RPC behavior, and all nine direct consumers are assigned. This does not promote the separate TiDB UniStore or `testutils` rows. |
...
`internal/logutil` completion supersedes the earlier descriptor/reflection seed. The exact three-artifact/237-line package has no source test, fixture, generated input, build variant, metadata file, or package-local build artifact. Rust now exposes process-wide/contextual logger selection, testing-sensitive warning assertions, replaceable-key span events and tags, and a generic protobuf `Hex` display wrapper. Regeneration gives all 927 checked-in generated messages `prost::Name` and emits a 691,743-byte descriptor set, allowing declaration-ordered recursive formatting and redaction of every bytes field without per-message tables. All 44 direct importers are classified; completed caller packages use the shared Rust logging/tracing/redaction owners, while root `tikv` and `internal/mockstore/mocktikv` retain their independent package claims.
