...
| `internal/client` | `src/store`, `src/pd`, transport | complete | Independently re-audited atomic receipt: [`internal-client-source-artifact-audit.md`](internal-client-source-artifact-audit.md). The immutable 19-artifact/6,924-line inventory, all 50 ordinary Go tests as direct independently executable Rust ports, `TestMain` harness disposition, every production/native representation surface, and all 32 direct importers are assigned. An exact-body correction removed 40 one-call aliases, activated 22 async bodies that ordinary `#[test]` wrappers had silently dropped, and added the previously documented-but-missing `TestPanicInRecvLoop` identity. Mechanical reconciliation now finds exactly 50 definitions with no test-to-test calls or package-local one-call aliases. No production divergence was exposed; exact Go normal/race suites, both 207-test source-identity matrices, complete 1,238/1,213 unit matrices, strict generation/check/Clippy/private rustdoc, and 51 doctests pass. Proxy selection remains owned by the separate complete `internal/locate` row. |
