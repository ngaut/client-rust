...
| `txnkv/rangetask` | `src/transaction/range_task.rs`, `src/transaction/client.rs` | seed | Atomic source inventory: `range_task.go` (358 lines) and `delete_range.go` (185 lines); no package-local Go test, generated input, fixture, or build variant. Rust transactional `delete_range` owns source `deleteRangeOneRegionMaxBackoff` (100,000-ms input, weighted by the same retry variables) instead of the generic region budget. The reusable native runner now covers ordered region-intersection discovery, default 128-region groups, bounded concurrent handlers, completed/failed counters, and shared first-error cancellation. A full receipt still requires the safe DeleteRange/keyspace adapter, caller-selected API, progress metrics/logging, notify-only DeleteRange, and every source behavior. |
