...
| `internal/resourcecontrol` | `src/resource_control.rs`, request contexts / resource manager proto | seed | Complete two-file inventory audited: `resource_control.go` and `resource_control_test.go`, with no generated/build/fixture artifacts. Native response accounting maps coprocessor/Batched-Cop scan-byte and KV-CPU extraction, including NextGen total-versus-processed compatibility and protobuf response sizing. Typed request extraction now maps source transactional/raw-write classification, Prewrite/Commit byte totals, peer store ID, encoded request size, cop identity, ordinary internal-request bypass, and NextGen analyze-coprocessor bypass through the Rust request boundary. Client-go's dynamic-wrapper-only access location, replica number, and predicted-read-byte fields remain explicit Rust defaults; controller interceptor selection/penalty/priority behavior and transport consumer/metrics integration remain required. |
...
| `metrics` | `src/stats.rs`, Prometheus instrumentation | seed | The error package now increments a native Prometheus transaction write-conflict counter; exact client-go namespace/subsystem/const-label registration and all remaining metric families/update sites still require the complete metrics receipt. |
...
| `txnkv` | `src/transaction/client.rs`, exports | unassessed | Public transaction client package; depends on all transaction subpackages. |
...
| `txnkv/txnlock` | `src/transaction/lock.rs`, plan lock resolution | unassessed | Include async resolve, shared locks, retries, and TTL. |
