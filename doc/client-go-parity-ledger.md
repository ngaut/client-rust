...
| `oracle/oracles` | `src/oracle/oracles.rs`, typed PD timestamp adapter and lifecycle tasks | complete | Independently re-audited atomic receipt: [`oracle-oracles-source-artifact-audit.md`](oracle-oracles-source-artifact-audit.md). All eight artifacts/2,063 lines, 13 ordinary tests as exact independently selectable Rust identities, `TestMain`, four direct concrete-package importers, and local/mock/PD allocation, refresh, validation, cancellation, and shutdown behavior are assigned. Differential gates additionally preserve raw-scope validation-flight identity and forward-only Unix-millisecond adaptive markers. |
...
The earlier coarse receipt is superseded by the 2026-08-26 atomic re-audit in [`oracle-oracles-source-artifact-audit.md`](oracle-oracles-source-artifact-audit.md). That receipt owns the immutable eight-artifact/2,063-line inventory, all 13 ordinary tests as exact independently selectable Rust identities plus `TestMain`, all four direct concrete-package importers, ten differential corrections, exact Go normal/race execution, and final pinned-nightly validation.
