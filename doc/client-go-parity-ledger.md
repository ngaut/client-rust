...
| `internal/client` | `src/store`, `src/pd`, transport | in-progress | Complete source inventory recorded: production `client.go`, `client_async.go`, `client_batch.go`, `client_collapse.go`, `client_interceptor.go`, `conn_batch.go`, `conn_monitor.go`, `conn_pool.go`, `legacy_codec.go`, and `priority_queue.go`; support `mockserver/mock_tikv_service.go`; tests `client_async_test.go`, `client_batch_test.go`, `client_fail_test.go`, `client_interceptor_test.go`, `client_test.go`, `priority_queue_test.go`, and goleak `main_test.go`; `OWNERS` is repository metadata only. Rust maps source connection settings, versioned pool lifecycle, request/error identity, native-future cancellation, forwarding, priority/batch collection, direct/forwarded stream grouping, response correlation, health feedback, load adaptation, stream recreation, idle retirement, pending diagnostics, and request/stream metrics. ResolveLock requests with empty `keys` and `txn_infos` now use one process-wide source-keyed singleflight `(region_id,start_version,is_async)`, a fixed 30-second physical timeout, independent caller timeouts, and cloned typed responses; lite and batch resolves bypass collapse. Native wrappers cover CoprocessorStream, BatchCoprocessor, and EstablishMPPConnection, eagerly reading the first item and retaining per-receive timeout/close ownership. Tonic exposes no grpc-go `GetState`; Rust therefore records the same five gauge labels from owned pool events (Idle registration, Connecting/Ready sends and stream opens, TransientFailure stream failures) and clears every label on pool removal instead of polling once per second. `legacy_codec.go` and `client_async.go` map respectively to Prost-owned decode buffers and Rust futures rather than compatibility/callback façades. Source execution-detail tracing now reconstructs the exact historical span tree and timing through an opt-in task-scoped sink on unary and batch transports, with request-plan capture for fan-out/retry tasks. The remaining package gate is complete dynamic stream-command/API-codec ownership with the separate `tikvrpc` and `internal/apicodec` packages; this row remains in progress until that integration and its source tests are closed. Proxy selection is correctly owned by `internal/locate`, not this transport receipt. |
