...
| `txnkv/txnlock` | `src/transaction/lock.rs`, `src/request/plan.rs`, transaction request contexts | seed | Atomic inventory: production `lock.go` (43) and `lock_resolver.go` (1,724); tests/support `lock_test.go` (63), `lock_resolver_test.go` (160), `main_test.go` (25), and `test_probe.go` (129), with no generated/build/fixture artifacts. Rust resolves ordinary exclusive locks, caches only source-determined committed/rolled-back statuses in the source 2,048-entry FIFO (rejecting conflicting final outcomes), and the native transaction client now shares that resolver context across its clones, explicit resolution, and cleanup operations. Prewrite carries per-shard `TxnSize`, and ordinary small-lock resolution follows client-go's configured 512-default lite threshold by resolving only the observed key and skipping an already-resolved primary. Cleanup also follows source `BatchResolveLocks` by suppressing an empty `BatchResolveLock` RPC after all locks were already deduplicated/clean; pessimistic cleanup uses PessimisticRollback rather than that batch protocol. Expired async-commit primaries now derive their outcome from source-shaped per-shard secondary checks, retain the final state, and resolve returned secondaries plus the primary exactly once per region; a missing secondary resolves only the primary, while a non-async response triggers forced sync-commit retry. Pessimistic locks use the source PessimisticRollback protocol (skipping the primary and defaulting missing for-update timestamps to `MaxUint64`), including the source primary-mismatch conversion for a pessimistic secondary, rather than ResolveLock. Transactional and snapshot Get, BatchGet, and Scan now use source `ResolveLocksForRead`: each transaction owns resolved/committed timestamp sets, retries carry them in the TiKV context, and final statuses are classified as ignore or read-through without blocking on secondary cleanup. All four source lock-resolver write RPCs (`CheckTxnStatus`, `CheckSecondaryLocks`, `ResolveLock`, and batch resolve) now carry the 20,000-ms TiKV execution duration. It retains resource-control/RU accounting through resolver plans and exposes source-shaped `extract_lock_from_key_error` and `extract_locks_from_key_error`; the plural helper expands a shared-lock wrapper only for a caller that can resolve every embedded holder, while direct resolver input rejects only `SharedLock`/`shared_lock_infos` wrappers. A concrete `SharedPessimisticLock` follows client-go's PessimisticRollback path. Rust still lacks client-go's single `KVStore` owner, so transaction/snapshot retry paths cannot yet share this cache store-wide. Full bounded async-pool/cleanup scheduling, observing, metrics, and source-test behavior remains required for a receipt. |
