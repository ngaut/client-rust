...
| `internal/apicodec` | `src/{common/errors,request/keyspace,request/mod,request/plan_builder,store/request,raw/client,raw/requests,transaction/{lock,requests,transaction}}.rs`, directly required proto/generated inputs | complete | Independently re-audited atomic receipt: [`internal-apicodec-source-artifact-audit.md`](internal-apicodec-source-artifact-audit.md). All seven artifacts/2,703 lines, eight required pinned protocol inputs, every production surface, all 17 assertion-bearing Go tests as direct Rust ports, the suite-runner/empty-test dispositions, and all 14 direct importers are assigned. The exact-body correction promotes the final one-call identity owner and removes one residual registered-test call; all 17 names now have one definition and no registered test invocation. No production divergence was exposed. Exact Go normal/race suites, both 207-test source-identity matrices, complete 1,237/1,212 unit matrices, strict check/Clippy/private rustdoc, and 51 doctests pass. Each high-level routing/raw/transaction consumer retains its own package status. |
| `internal/client` | `src/store`, `src/pd`, transport | complete | Independently re-audited atomic receipt: [`internal-client-source-artifact-audit.md`](internal-client-source-artifact-audit.md). The immutable 19-artifact/6,924-line inventory, all 50 ordinary Go tests as direct independently executable Rust ports, `TestMain` harness disposition, every production/native representation surface, and all 32 direct importers are assigned. Exact-body corrections removed 40 one-call aliases, activated 22 async bodies that ordinary `#[test]` wrappers had silently dropped, added the previously documented-but-missing `TestPanicInRecvLoop` identity, and removed two residual registered-test calls. Mechanical reconciliation now finds exactly 50 definitions with no registered test calls or package-local one-call aliases. No production divergence was exposed; exact Go normal/race suites, both 207-test source-identity matrices, complete 1,237/1,212 unit matrices, strict generation/check/Clippy/private rustdoc, and 51 doctests pass. Proxy selection remains owned by the separate complete `internal/locate` row. |
...
| `internal/locate` | `src/error.rs`, `src/region.rs`, `src/region_cache.rs`, `src/region_request.rs`, `src/pd`, `src/request`, `src/store`, `src/traffic.rs` | complete | Independently re-audited complete receipt below and full 17-artifact/production-symbol/147-declaration/dependency/consumer mapping in [`internal-locate-source-artifact-audit.md`](internal-locate-source-artifact-audit.md). Whole-package body scanning corrected both a six-name forwarding macro and later one-call aggregate-helper aliases missed by the earlier directness claims. All 142 assertion-bearing Go declarations now have independently selectable ordinary Rust tests that perform source-specific actions and assertions, with no forwarding macro, registered test-to-test call, or exact identity consisting only of a helper call; `TestMain` and the four testify suite runners have explicit dispositions. The stronger sender tests fixed read-TS error erasure and restored complete RPC-context/backoff diagnostics, including endpoint-specific access indexes, proxies, full gogo protobuf text, and an actual hintless-NotLeader request path. Exact Go normal/race suites, source-derived Rust gates with 328 plus one skip/324 plus five skips, 1,274/1,249 canonical Rust tests, strict check/Clippy/private rustdoc, and 51 doctests pass. Region coding and indexes, TTL/GC/refresh, store discovery/liveness/health, replica and proxy selection, sender retry/error precedence, runtime diagnostics, network accounting, configurable PD region-meta circuit breaking, and native async ownership are accounted for. Raw, snapshot, transaction, root `tikv`, and other consumers retain independent statuses. |
...
| `integration_tests` (root test package) | direct source ports across completed Rust production owners | complete | Atomic receipt: [`integration-tests-source-artifact-audit.md`](integration-tests-source-artifact-audit.md). All 36 root artifacts/15,938 lines are assigned: 31 test/support files, `go.mod`/`go.sum`, and three real-cluster fixtures. Mechanical enumeration finds 181 receiver methods, 23 suite-only top-level runners, nine standalone behavioral tests, and `TestMain`. Exactly 190 behavioral declarations have one direct Rust owner: 154 root integration identities, 34 already-direct `txnsnapshot` owners, and two already-direct `rangetask` owners, with no registered test-to-test forwarding. The complete port exposed and fixed stale ForceLock wakeup results, txn-file retry-flag loss after primary region regrouping, exact wrapping prewrite minimum-commit timestamp selection, mixed shared/exclusive scan-lock ordering, and max-execution arbitration against the effective context wait. Source build tags and skips are explicit. Exact Go normal/race suites and both Rust feature matrices pass; the final workspace matrices pass 1,393/1,393 no-default and 1,357/1,357 all-feature tests. The nested `integration_tests/raw` package retains its separate `rawkv` receipt. |
...
| `unistore` (native crate) | independent `unistore` workspace crate | complete | Re-audited atomic receipt: [`unistore-crate-completion-audit.md`](unistore-crate-completion-audit.md). All six crate artifacts/3,893 lines, 80 public declaration points, 43 unit tests, three external-consumer tests, dependencies, and native consumers are assigned. The 23 mocktikv source identities are direct test bodies; the single reusable deadlock graph serves the live source-mapped engine. |
...
| `oracle` | public `src/oracle.rs` interface/helper owner | complete | Re-audited atomic receipt: [`oracle-source-artifact-audit.md`](oracle-source-artifact-audit.md). The sole 157-line artifact, all 48 importers, every contract/helper/error branch, exact Go-duration overflow, and no-source-test disposition are assigned. |
| `oracle/oracles` | `src/oracle/oracles.rs`, typed PD timestamp adapter and lifecycle tasks | complete | Reopened and independently re-audited atomic receipt: [`oracle-oracles-source-artifact-audit.md`](oracle-oracles-source-artifact-audit.md). All eight artifacts/2,063 lines, 13 ordinary tests as exact independently selectable Rust identities, `export_test.go`, `TestMain`, four direct concrete-package importers, and local/mock/PD allocation, refresh, validation, cancellation, and shutdown behavior are assigned. The direct ports use source-faithful unseeded constructors/cache overwrites. Differential gates preserve nonzero shrink-request control flow, raw-scope validation-flight identity, and forward-only Unix-millisecond adaptive markers. |
| `rawkv` | public `src/raw`, plus native keyspace/request/region/metrics owners | complete | Independently re-audited atomic receipt: [`rawkv-source-artifact-audit.md`](rawkv-source-artifact-audit.md). All eight package/external-test/config artifacts (2,743 lines), all 27 assertion-bearing suite methods as direct independently selectable Rust ports, three suite-runner dispositions, `TestMain`, every setup/probe/config hook, all production methods, and three direct external importers are assigned. The former 23-name forwarding macro and four store-helper aliases are gone; mock/live splits mutate one fixture after writes, including all 20 live scan split points. Exact Go package normal/race suites pass in legacy and NextGen modes, and the separate integration/raw normal/race suites pass. The stronger ports found no production divergence. The ordinary-build injected constructor and batch-size probe remain public; deterministic ports execute every source assertion, including all three 20,480-pair scan/delete/checksum cases. Root `tikv` and `internal/mockstore/mocktikv` retain independent receipts. |
...
| `tikv` | public `src/tikv.rs` facade plus native `src/{pd,region_cache,request,store,transaction,stats}.rs` owners | complete | Exact-body re-audited atomic receipt: [`tikv-source-artifact-audit.md`](tikv-source-artifact-audit.md). All 17 root artifacts/3,895 lines, every production facade/worker/control path, all 12 suite methods plus the lifecycle test as 13 direct `source_go_tikv_kv_test_*` bodies, both harness dispositions, typed support probes, exact metric sites, pinned `gp` behavior, and all 35 direct importers are assigned. The latest red/green audit removes artificial Spool delay/active-work serialization, gates PD's cluster safe-TS shortcut on global transaction scope, and restores PD's legacy `safe_point_version=v2` keyspace-GC fallback with new-field precedence. Earlier corrections retain region-cache-before-client close ordering, primitive safe-TS probe overwrites without weakening updater monotonicity, immediate running-scatter header errors, and shared TTL-capped split-lock retry ownership. Exact Go normal/race suites, 13 focused identities and all 37 root tests in both Rust configurations, complete 1,398/1,362 matrices, strict check/Clippy/private rustdoc, and 51 doctests pass. Production control-plane calls use mock/loopback validation here; root `txnkv`, external integration tests, and the completed live-cluster differential gate retain independent receipts. |
...
| `txnkv/rangetask` | `src/transaction/range_task.rs`, `src/transaction/client.rs`, `src/transaction/requests.rs`, `src/stats.rs` | complete | Independently re-audited atomic receipt: [`txnkv-rangetask-source-artifact-audit.md`](txnkv-rangetask-source-artifact-audit.md). Both production files/543 lines, the sole 265-line external test, its suite-runner disposition, both behavioral methods as independently named Rust ports, all 200 success and 348 injected-failure executions, every production surface, and all five direct consumers are assigned. A red/green differential fixed progress ticks canceling and indefinitely restarting in-flight PD region discovery. Exact Go normal/race suites, both nine-test focused Rust configurations, complete 1,407/1,382 matrices, strict check/Clippy/private rustdoc, and 51 doctests pass. Downstream GC, split/scatter, and pipelined transaction algorithms retain their own package rows. |
| `txnkv/transaction` | `src/transaction/{transaction,txn_file,client,requests,lowering,buffer,unionstore}.rs`, request/error/PD integration | complete | Re-audited atomic 16-artifact/11,766-line receipt: [`txnkv-transaction-source-artifact-audit.md`](txnkv-transaction-source-artifact-audit.md). All 33 original Go test declarations own direct independently selectable Rust actions and assertions, with two additional source-uncovered boundary regressions. The latest red/green audit restores `LockKeysFunc` callback registration after aggressive-lock applicability preflight and signed, Go-duration-saturated lock-wait subtraction. Root integration ports additionally corrected effective-wait max-execution arbitration, wrapping prewrite minimum timestamps, and transaction-file retry-state retention after regrouping. `Transaction` exposes and commits one authoritative staged `MemDb`; pipelined mode rotates that same store through bounded generations, direct remote reads, typed flush/status/TTL lifecycles, commit, and rollback. API-v2 retains logical keys and lowers them exactly once. Large 2PC groups use source-exact split, per-region scatter, operator wait, and post-wait cache invalidation with public process-wide thresholds. Exact Go normal/race gates, all 33 focused ports in both configurations, and complete 1,395/1,359 Rust matrices pass. |
| `txnkv/txnlock` | `src/transaction/lock.rs`, `src/transaction/{client,requests,transaction}.rs`, `src/request/{plan,plan_builder}.rs`, `src/{stats,tikv}.rs` | complete | Re-audited atomic receipt: [`txnkv-txnlock-source-artifact-audit.md`](txnkv-txnlock-source-artifact-audit.md). All two production and four test/support artifacts (2,144 lines), five independently named ordinary test ports plus `TestMain`, production symbols, direct consumers, lifecycle owners, and validation gates are assigned. Red/green differential tests corrected cached async-primary metadata loss, missing region-wide pessimistic rollback, rollback of live pessimistic locks, and missing explicit-Lite point cleanup. Source-public cache/semaphore constants, exact consumer option scoping, duplicate async cleanup suppression, bounded pool ownership, read hints, GC, NextGen async requests, resource context, and shutdown now match the pin. Exact Go normal/race suites and final 37-test focused Rust matrices pass. Snapshot retry-class consumption remains owned by `txnkv/txnsnapshot`. |
| `txnkv/txnsnapshot` | `src/transaction/{snapshot,sync_snapshot,snapshot_stats,transaction,buffer,requests,client}.rs`, `src/request/{plan,plan_builder,shard}.rs`, request context and metrics | complete | Re-audited atomic receipt: [`txnkv-txnsnapshot-source-artifact-audit.md`](txnkv-txnsnapshot-source-artifact-audit.md). All five source artifacts/2,317 lines, every production/probe surface, ten direct importers, two indirect probe consumers, and all 40 external `Test*` declarations are assigned. The six top-level suite runners map to Rust harnesses; all 34 behavioral methods now have direct independently selectable Rust bodies with exact zero-case and source/NextGen skip dispositions. A follow-up integrity audit removed 21 macro-forwarded identities and nine handwritten evidence aliases without production churn; eight aliases had called registered tests and the ninth shared another package's scenario helper. Mechanical comparison reports zero missing, extra, duplicate, forwarding, or registered test-to-test calls. Executable ports include the exact 1/256/257/768-row scan matrix and 123/456 region splits, the 5×30 shared-snapshot workload, response/pair lock distinctions, callback enabled/disabled behavior, and all ten pipelined-MemDB methods. The production re-audit fixed eager multi-region scan row loss (691/768 before the fix), wait-all snapshot fanout/error/fork ownership, one-shard no-fork behavior, post-lock physical route reselection, and the previously dormant async-BatchGet configuration. Exact Go normal/race suites, focused Rust matrices, 1,275/1,250 workspace matrices, strict checks, and 51 doctests pass. Root `tikv` retains the concrete GC visibility provider and live integration gates; no other row is promoted. |
...
| `util` | shared public `src/util`, plus native tracing/traffic/PD/transaction integrations | complete | Re-audited atomic receipt: [`util-source-artifact-audit.md`](util-source-artifact-audit.md). All 13 artifacts/3,478 lines, all 30 original tests plus `TestMain`, exported/context/concurrency surfaces, and 58 direct importers are assigned. Every test now has an independently named port and assertion-level mapping; strengthened clone, pool-task, GC-time, range, and rate-limit gates close the former grouped-evidence weakness. Custom DNS, execution/RU/pool/scan/write/time details, failpoints, recovery, request identity, rate limiting, timestamp sets, and intercepted PD waits preserve source behavior without promoting callers. |
...
| `util/collectors` | public `src/util/collectors.rs`, generated channelz client/server/messages | complete | Independently re-audited atomic receipt: [`util-collectors-source-artifact-audit.md`](util-collectors-source-artifact-audit.md). Both artifacts/1,189 lines, all six Go tests as exact independently selectable Rust ports, every production surface, the external generated dependency, and the no-benchmark/example/variant/importer boundary are assigned. A red/green overlapping-scrape regression makes cumulative fetch errors observable immediately at the RPC failure site like client-go instead of only after the failing walk completes. |
...
All 38 top-level kvproto schemas are now byte-identical to `kvproto@059694ae4472276644613acccefa24cbc89d959f`; together with the independent channelz schema and 17 vendored imports, the 56-input generator emits 41 protocol modules, `mod.rs`, and a 728,127-byte descriptor reproducibly into the standalone `tikv-client-kvproto` crate. `tikv_client::proto` re-exports those exact types. Generation stages a clean output and removes stale generated artifacts; the stable normalized generated-output SHA-256 manifest is `b77f2aa05bc26eedd23c6b8ba1896edae9f5c72b7c5a090f1e5809999e31d8fa`. No source package, tracked non-package artifact, generated family, example, integration fixture, or final live gate remains unassigned.
...
The 2026-08-26 independent re-audit reopened the exact seven-artifact/2,703-line package boundary, reran the original Go suite and race suite with Go 1.25.12, and replaced grouped test references with one independently named Rust port for each of the 17 assertion-bearing Go tests. `TestCodecV2` is only the testify suite runner, while `TestV1DecodeBucketKey` is empty at the source pin. The stronger cases found no additional production divergence; they directly replay the exact constructor/range, six-region epoch, nested key-error, response-command, empty-MVCC, MPP, and three bucket-edge tables.

Validation on `nightly-2026-08-22`:

    go test ./internal/apicodec -count=1
    go test -race ./internal/apicodec -count=1
    # passed
    ...
    # main library: 1,041 passed; 0 failed; 1 ignored; all companion and doctest targets passed

    cargo +nightly-2026-08-22 test --quiet --lib --all-features
    # 1,038 passed; 0 failed; 1 ignored
...
The 2026-08-26 re-audit ran the exact package with Go 1.25.12: normal and race suites passed in 68.669s and 70.101s. Mechanical reconciliation finds exactly 142 source tests, zero missing, zero duplicates, and zero forwarding macros. On `nightly-2026-08-22`, the complete source-derived Rust filters pass in both configurations, and canonical matrices pass 1,328 no-default workspace tests with two configured skips plus 1,302 all-feature library tests with six configured skips. The stronger `TestRegionRequestValidateReadTS` port exposed one production divergence: dispatch stringified oracle errors, destroying the concrete source error identity. Public `Error::Oracle` now preserves downcastable `FutureTimestampReadError` and `LatestStaleReadError`, with all eight sender rows and an ordinary downstream-crate gate passing. Strict all-target check/Clippy/private rustdoc, all 51 doctests, rustfmt, exact inventory/declaration/importer reconciliation, source identity, and whitespace checks pass. Deterministic routing, cache, selector, error, coding, and lifecycle behavior plus completed loopback Tonic transport establish this package boundary without a live cluster. Raw, snapshot, transaction, root `tikv`, and final cross-client cluster workflows retain independent gates.
...
The earlier coarse receipt is superseded by the 2026-08-26 atomic re-audit in [`internal-client-mockserver-source-artifact-audit.md`](internal-client-mockserver-source-artifact-audit.md). That receipt owns the immutable one-artifact/196-line inventory, no-test/support boundary, all five direct importers, exact Go 1.25.12 compilation, both runtime differential corrections, and final pinned-nightly validation.
...
[`tikvrpc-interceptor-source-artifact-audit.md`](tikvrpc-interceptor-source-artifact-audit.md).
That receipt owns the immutable three-artifact/335-line inventory, the original
test and goleak harness, all seven direct importers, exact Go and race execution,
both public API corrections, the transaction commit/read consumer port, and
final pinned-nightly validation.
...
The earlier coarse receipt is superseded by the 2026-08-26 atomic re-audit in [`internal-kvrpc-source-artifact-audit.md`](internal-kvrpc-source-artifact-audit.md). That receipt owns the immutable one-artifact/82-line inventory, no-test/support boundary, both direct importers, exact Go 1.25.12 compilation, the split-region consumer correction, and final pinned-nightly validation.
...
The earlier coarse receipt is superseded by the 2026-08-26 atomic re-audit in [`oracle-source-artifact-audit.md`](oracle-source-artifact-audit.md). That receipt owns the immutable one-artifact/157-line inventory, all 48 direct importers, exact Go execution, the no-source-test disposition, Go-duration overflow correction, and final pinned-nightly validation.
...
The earlier coarse receipt is superseded by the 2026-08-26 atomic re-audit in [`internal-latch-source-artifact-audit.md`](internal-latch-source-artifact-audit.md). That receipt owns the immutable five-artifact/758-line inventory, all four ordinary tests plus `TestMain`, all four direct importers, exact Go 1.25.12 execution, the shared-scheduler shutdown correction, and final pinned-nightly validation.
...
The earlier coarse receipt is superseded by the 2026-08-26 atomic re-audit in [`error-source-artifact-audit.md`](error-source-artifact-audit.md). That receipt owns the immutable two-artifact/497-line inventory, the sole original test, all 58 direct importers, exact Go 1.25.12 execution, differential corrections, and final pinned-nightly validation.
...
The earlier coarse receipt is superseded by the reopened 2026-08-26 atomic re-audit in [`oracle-oracles-source-artifact-audit.md`](oracle-oracles-source-artifact-audit.md). That receipt owns the immutable eight-artifact/2,063-line inventory, all 13 ordinary tests as exact independently selectable Rust identities plus source-faithful `export_test.go` support and `TestMain`, all four direct concrete-package importers, exact Go normal/race execution, and final pinned-nightly validation.
...
Rust implementation is `src/oracle/oracles.rs`, exposed through `src/oracle.rs`. It provides local and mock oracles with exact physical/logical allocation, expiration, stale-time and externally-monotonic timestamp behavior. `PdOracle` initializes the global scope, maintains monotonically increasing per-scope cache entries, serves stale timestamps, and implements low-resolution refresh, manual configuration, adaptive shrinking/recovery, source-exact raw-scope validation singleflight, one-retry protection for timestamps from another PD client, and native dropped-future cancellation safety. Adaptive short-read markers are forward-only Unix-millisecond values like the source; empty and explicit `global` validation scopes own distinct flights while sharing the normalized global timestamp cache. `PdClientTimestampSource` connects this behavior to the existing PD client; `src/pd/client.rs`, `src/pd/retry.rs`, and `src/pd/cluster.rs` add the source-required GetMinTS and external-timestamp RPCs. Existing generated PD bindings are sufficient and unchanged.

`src/stats.rs` records the three source-owned observability side effects: timestamp-future wait duration, validation fetches from PD, and the active low-resolution update interval. The full registration/inventory of client-go's broader `metrics` package remains a separate ledger claim. Go contexts map to Rust future cancellation: canceling a waiter drops only that wait while the shared PD request continues for the other waiters. The test-only empty-PD constructors/hooks map literally to an unseeded injectable `PdTimestampSource`, direct TSO/arrival overwrite, explicit cache seeding in the owning test body, and deferred refresh-loop start. The Rust task test verifies the refresh task finishes after `close`, the native equivalent of the Go goleak assertion.

Focused tests cover every original local and PD scenario: 100,000 distinct local timestamps; expiration/until-expired boundaries; stale timestamp errors; cache monotonicity under 100 concurrent setters; refresh scheduling; adaptive state transitions and manual updates; invalid/latest/future read timestamps; stale-read-only adaptation; shared validation requests; a canceled shared waiter; and the different-client, older-singleflight retry path. Mechanical declaration reconciliation reports 13 Go tests, 13 exact Rust definitions, and zero missing, extra, or duplicate identities. The latest red/green regression preserves client-go's rule that nonzero shrink requests run only unadjustable/adapting checks, while earlier gates retain raw empty-versus-global validation-flight keys and adaptive-marker precision/monotonicity. No unistore fixture is required: all package-owned PD behavior is represented by deterministic timestamp sources, while real PD RPC interoperability belongs to the transport/client integration packages.

Exact Go normal and race suites pass under Go 1.25.12. On `nightly-2026-08-22`, all 13 exact source ports and all 30 package tests pass in both feature configurations; complete nextest matrices pass 1,399/1,399 without default features and 1,363/1,363 with all features. Strict check, documentation, rustfmt, source-identity, and whitespace gates pass. No real-cluster test is required for the package-owned cache/allocation/validation state machine; concrete PD transport compatibility, high-level oracle wiring, and broader metrics registration remain assigned to their independent completed receipts.
...
The independently re-audited immutable boundary is 15 artifacts/4,813 lines: eight production files (`memdb.go`, `memdb_art.go`, `memdb_rbt.go`, `membuffer_snapshot.go`, `mock.go`, `pipelined_memdb.go`, `union_iter.go`, and `union_store.go`), six test/support/benchmark files, and `OWNERS`. `memdb_norace_test.go` is the sole `!race` variant. There is no `doc.go`, generated source/input, fixture, example, platform source, or package build artifact. The child `arena`, `art`, and `rbt` packages retain their own complete receipts; this receipt composes their public parent-facing behavior only.
...
The source declares 50 top-level ordinary tests, five named subtests inside `TestBatchedSnapshotIterEdgeCase`, `TestMain`, and 12 benchmarks. Splitting the subtest parent yields 54 executable test cases. Rust now has 54/54 exact `source_go_internal_unionstore_<file>_<GoNameOrSubtestPath>` definitions and 12/12 exact benchmark-contract definitions, with zero missing, extra, or duplicate identities. The tests retain deterministic set/get/delete, bounds, forward/reverse union merging, flags, stages, checkpoints, reset/history, snapshots and batched edge cases, hooks/cache metrics/limits, unsupported pipelined errors/panics, thresholds/skipping/blocking/generations/read precedence, and source-scale randomized shapes. `TestMain` maps to explicit worker completion and full-process suite completion.

The re-audit found one additional production divergence. Rust reused the pipelined point-read cache during `BatchGet`; client-go always sends every non-local key to the remote buffer getter and refreshes that cache for later point reads. The new regression was red with stale value `one` after the remote value became `two`. Authoritative `MemDb`, public `PipelinedMemDb`, and the transaction consumer now preserve the source split: batch reads refresh, point reads reuse.

The same `MemDb` core is now the authoritative public `transaction::buffer` mutation store. Ordinary transactions use its staging, flags, iteration, and accounting directly; pipelined transactions activate managed generation state on that exact instance rather than draining into a second buffer. Public async transaction behavior, transport, commit/rollback, and real TiKV differential coverage remain accounted for by the separate complete `txnkv/transaction` receipt rather than being duplicated in this parent-package claim.

Exact Go normal/race suites pass under Go 1.25.12. On `nightly-2026-08-22`, all 66 exact source test/benchmark identities and all 88 package tests pass in both feature configurations. Complete nextest gates pass 1,410/1,410 without default features and 1,385/1,385 with all features. Strict workspace check/Clippy, private rustdoc, 51 doctests, rustfmt, exact 54/54 test plus 12/12 benchmark identity reconciliation, and whitespace checks pass. No real cluster is required for this deterministic buffer package; its transaction integration regression exercises the actual authoritative consumer path.
...
The completed `internal/unionstore`, `internal/latch`, `internal/locate`, `internal/client`, `internal/apicodec`, `tikvrpc`, `config/retry`, error, config, and KV packages supply this package's prerequisites. The reusable `unistore` crate remains optional test substrate and is already exercised by the unionstore dependency. Rust's source-default ART implements physical `RemoveFromBuffer`, matching the public MemBuffer/RBT contract and supporting transaction lock-cache cleanup; only pipelined MemDB retains its source-declared unsupported panic.

Native ownership replaces exposed unionstore/committer pointers, Go contexts/goroutines/wait groups, callback-style async probes, and dynamic mutation containers with encapsulated buffers, typed futures/tasks, lifecycle hooks, deterministic mock dispatch, and owned mutation vectors. These are integration decisions rather than missing behavior. The independent `txnkv`, `txnkv/txnlock`, `txnkv/txnsnapshot`, root `tikv`, and integration receipts are not promoted by this transaction-package receipt.

Final validation on `nightly-2026-08-22`: exact pinned Go normal/race baselines pass; all 33 one-to-one Rust identities pass in both feature configurations; 1,373 no-default workspace tests and 1,352 all-feature library tests pass with one intentional skip in each matrix; strict generated-code checking, all-target/all-feature compilation, Clippy, rustfmt, private-item rustdoc, all 51 doctests, and `git diff --check` pass. Mechanical comparison finds 33 pinned source declarations and 33 independently named Rust tests with no missing or extra name. The four original package test files require no live cluster; cross-client real-TiKV differential validation remains a high-level repository gate.

## Complete re-audit: `internal/client`

The prior append-only amendment history is superseded by the 2026-08-25 atomic re-audit in [`internal-client-source-artifact-audit.md`](internal-client-source-artifact-audit.md). That audit owns the immutable source identities, all 51 original test declarations, native Tonic/Prost integration decisions, and the corrected Debug configuration, malformed-response prefix processing, same-stream panic recovery, per-connection recovery epochs/lock ordering/concurrency, admission-inclusive deadlines and close wakeup, shared batch timing, send-loop preservation, idle deadline, randomized diagnostics, exact custom-policy JSON, response watermark, terminal outcome, timeout cancellation, and worker-drop behavior.

Final local gates on `nightly-2026-08-22` pass 38 focused source-derived BatchCommands tests, clean deterministic proto regeneration, workspace all-target/all-feature checking and Clippy, rustfmt, 771 no-default-feature tests, 761 all-feature library tests, private-item rustdoc, 51 doctests, diff checks, and exact 19-artifact/51-test source inventory comparison. The host has no Go executable, so the original Go suite was not rerun; its package-owned scenarios are mapped individually in the audit. The row is `complete`; no superseded “in-progress” amendment remains authoritative.

## Complete re-audit: `rawkv`

The earlier retry-only amendment is superseded by the atomic receipt in [`rawkv-source-artifact-audit.md`](rawkv-source-artifact-audit.md). The re-audit mechanically rechecked all eight artifacts/2,743 lines, all 31 executable test declarations and support hooks, every production method, and all direct consumers. It ports every package, mock-integration, and optional-live assertion into always-run Rust tests, including the 20,480-pair scan/delete/checksum cases, exact table-driven scans and deletes, empty values, TTL, CAS, column families, store replacement, and probe surfaces.

Production parity now includes a public ordinary-build injected RawClient constructor and batch-size probe; one-boundary-region-at-a-time DeleteRange and Checksum with fresh 20,000-ms backoffers; first-error stop for DeleteRange; source-ignored RawChecksum string errors and RawBatchGet/RawScan pair errors; source-exact public error text; and source behavior for malformed over-limit scan responses. The complete retry topology remains the one described by the original amendment: concurrent BatchGet/BatchDelete/BatchPut fork/cancel/final-child accounting, and fresh point/page/region owners elsewhere. Final gate evidence is recorded in the atomic receipt; no rawkv package inventory or integration gate remains open.

## Historical package amendments
...
`txnkv/txnsnapshot` completion supersedes every earlier seed remainder above. The exact five-artifact/2,317-line receipt closes one-boundary-region forward/reverse routing across empty regions, one shared region/lock retry owner per operation or refill, async future fanout/cancellation, pair-locked BatchGet key narrowing with clean-result retention, latest point-lock omission, request-source propagation, source-exact 10-GiB cache accounting/eviction, command/region/SLI metrics, post-response store visibility calls, and all probe equivalents. It also corrects the stale protobuf claim: pinned `ExecDetailsV2` does contain `PoolTaskDetails`, now fully merged, cloned, exposed, and formatted. All direct and indirect consumers and external test matrices are assigned in `txnkv-txnsnapshot-source-artifact-audit.md`; the independent re-audit gives all 34 behavioral suite methods one-to-one Rust identities, preserves the six harness-only dispositions and exact skips, and passes exact Go normal/race suites, both focused Rust configurations, 1,406 no-default workspace and 1,381 all-feature library tests, strict checks, and 51 doctests. Root `tikv` still owns the concrete GC-state visibility provider and the live-cluster differential gate.

`metrics` completion supersedes every earlier seed statement that Rust SummaryVec collectors were represented by histograms or that only a partial fixed-name registry existed. The exact two-artifact/1,726-line receipt closes all 98 declarations/97 initialized collectors, 151 declarations/149 initialized shortcuts, source-exact names/help/labels/buckets/subsystems/constant labels, no-quantile summary exposition, reinitializable process-wide production integration, commit/SLI helpers, and all 15 stale-store vectors while preserving the three pinned nil globals. Mechanical comparison finds zero metadata/order mismatches and all 34 direct importers are assigned. Final local gates pass 610 active tests in both complete library configurations, 50 doctests, all-target compilation, Clippy, rustdoc, format/diff, and exact source identity. `rawkv`, root `tikv`, and external integration behavior retain their own rows.

`util/collectors` completion supersedes the earlier blocked runtime note. The independent re-audit keeps the exact two-artifact/1,189-line boundary and replaces descriptive test names with six source-file-qualified, independently selectable Rust ports. Their fixtures now preserve every populated Go field, including mapped IPv4 bytes and unasserted state/timestamp/call data. Production comparison exposed one concurrent-observability divergence: Rust previously accumulated fetch errors privately until a complete graph walk returned, whereas client-go increments the shared atomic at each failed RPC. The red regression observed one error instead of two during overlapping scrapes; shared immediate counters now match source. Exact Go normal/race tests, both focused configurations, 1,411/1,386 complete Rust matrices, strict check/Clippy/private rustdoc, 51 doctests, and exact inventory/declaration/importer gates pass. The Prometheus-facing call remains synchronous, concurrent, live, and uncached while the dedicated worker executes Tonic RPCs; the package has no direct pinned importers and promotes no caller or generated-root claim.

`internal/logutil` completion supersedes the earlier descriptor/reflection seed. The exact three-artifact/237-line package has no source test, fixture, generated input, build variant, metadata file, or package-local build artifact. Rust now exposes process-wide/contextual logger selection, testing-sensitive warning assertions, replaceable-key span events and tags, and a generic protobuf `Hex` display wrapper. Clean regeneration emits 977 `prost::Name` implementations across 39 files and a 728,127-byte descriptor set containing 1,029 messages, allowing declaration-ordered recursive formatting and redaction of every bytes field without per-message tables. All 44 direct importers are classified; completed caller packages use the shared Rust logging/tracing/redaction owners, while root `tikv` and `internal/mockstore/mocktikv` retain their independent package claims.

Root `util` completion replaces the unassessed row with an exact 13-artifact/3,478-line receipt. The shared Rust owner now covers explicit custom-DNS resolution, every execution/commit/lock/RU/pool/scan/write/time aggregate and source string, all context keys, failpoint gating, recovery and GC-time compatibility, request/resource/session identity, cancellation-aware rate limiting, timestamp sets, and complete PD delegation with scoped wait accounting. All 30 original tests and the goleak lifecycle contract are assigned, and all 58 exact direct importers are classified without promoting root `tikv`, root `txnkv`, or `internal/mockstore/mocktikv`.
...
## `internal/mockstore/mocktikv` completion receipt

The exact source inventory is ten production files, three ordinary test files, and one goleak harness: 14 artifacts and 6,689 lines. There is no `doc.go`, build/platform variant, generated input/output, fixture, package build file, benchmark, example, or package metadata artifact. The immutable hashes, every production surface, all 23 ordinary test declarations, the lifecycle contract, and all nine direct consumers are recorded in [`internal-mockstore-mocktikv-source-artifact-audit.md`](internal-mockstore-mocktikv-source-artifact-audit.md).

The reusable `unistore` crate now owns the complete in-process MVCC/raw state engine, directory-backed close/reopen state, deadlock graph, and Go-compatible record/checksum formats. The hidden client module owns cluster topology, sessions, PD state, every source-handled unary/stream RPC command, failpoints, error conversion, and construction/close integration. This corrects the source audit's final edge cases: reflected Go CRC64 ECMA, empty-info MVCC start-TS misses, source no-error raw delete-range responses, previous-region lookup, and quotient/remainder `SplitKeys` grouping. The native crate closes through its own receipt below; TiDB's separate server package is explicitly `not-applicable`, not partially claimed. The dependent `testutils` facade is completed by its own atomic receipt below.

## `testutils` completion receipt

The exact source inventory is the sole 61-line `testutils/mockstore.go` production artifact. There is no package doc, Go test/support artifact, fixture, build/platform variant, generated input/output, benchmark, example, metadata artifact, build tag, or generation directive. Its immutable hash, all 12 exported aliases/functions, native integration decisions, and all 14 direct consumers are recorded in [`testutils-source-artifact-audit.md`](testutils-source-artifact-audit.md).

The ordinary-build public `tikv_client::testutils` module is a zero-state facade over the completed cluster and mocktikv owners. It exports the cluster/coprocessor/MVCC/client/session contracts, the concrete PD support type, the factory, all three source bootstrap helpers, and the field-preserving locked-error contract. The source's interface returns map to native Rust traits or concrete complete implementors; idiomatic acronym casing does not alter behavior. The source-derived conformance test constructs every alias and executes every function, while no-feature external tests implement the handler and commit/read a transaction through the factory. Concrete behavior remains proved by mocktikv's package receipt. Integration consumers retain their independent ownership and final differential gates.
