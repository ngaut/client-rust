...
`util/collectors` completion supersedes the earlier blocked runtime note. The exact two-artifact/1,189-line receipt closes all source APIs, six original tests, graph traversal/filter/deduplication, all optional channel/socket/error metric families, and the exact grpc-go v1.82.1 channelz generated dependency. The Prometheus-facing call remains synchronous, concurrent, and uncached while a dedicated worker runtime executes Tonic RPCs; a byte-identical descriptor gate proves the vendored schema. The package has no direct pinned importers and promotes no caller or generated-root claim.
