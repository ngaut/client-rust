...
| `tikvrpc/interceptor` | public `src/interceptor.rs`, transaction/snapshot dispatch plans, downstream API gate | complete | Re-audited atomic receipt: [`tikvrpc-interceptor-source-artifact-audit.md`](tikvrpc-interceptor-source-artifact-audit.md). All three artifacts/335 lines, the original test plus `TestMain`, every production symbol, and all seven direct importers are assigned. Rust now publishes the source mock manager and downstream implementors need only the source name/wrap contract; exact commit/read propagation and all chain branches are executable. |
