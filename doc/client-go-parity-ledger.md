...
| `internal/client` | `src/store`, `src/pd`, transport | in-progress | Complete source inventory recorded: production `client.go`, `client_async.go`, `client_batch.go`, `client_collapse.go`, `client_interceptor.go`, `conn_batch.go`, `conn_monitor.go`, `conn_pool.go`, `legacy_codec.go`, and `priority_queue.go`; support `mockserver/mock_tikv_service.go`; tests `client_async_test.go`, `client_batch_test.go`, `client_fail_test.go`, `client_interceptor_test.go`, `client_test.go`, `priority_queue_test.go`, and goleak `main_test.go`; `OWNERS` is repository metadata only. Rust now maps source connection settings for compression, HTTP/2 keepalive, flow-control windows, and `grpc-connection-count`: it opens the configured number of TiKV channels per store and selects them with client-go's pre-increment round-robin order. Gzip response decoding remains enabled. `TikvConnect` now retains the full caller `TiKvClient` section, so each per-store BatchCommands worker observes the configured enablement, queue size, collection policy, overload threshold, and concurrency limit instead of reconstructing defaults. `KvRpcClient::dispatch_with_forwarded_host` is the source forwarding transport primitive: it groups batchable requests by the supplied host and applies `tikv-forwarded-host` to every supported unary RPC without mutating its protobuf. The dynamic `Dispatch` wrapper retains the selected physical target and logical forwarded host across sharding; each pooled `BatchCommands` stream carries Go's direct/forwarded metadata and its `tikv-batch-conn-index`. `src/store/batch.rs` now owns the seed batch pipeline: priority selection, direct/forwarded grouping, caller response handoff, publication-before-send, exact-ID send failure, per-host receive failure, response correlation/highest-ID accounting, same-pool-slot direct/forwarded streams per nonempty flush, source turbo-batch policy state, exact terminal completion of pre-selection cancellation, normal-priority max-concurrency admission based on published pending IDs (with source high-priority bypass), and cancellation-owned stream recreation using the source `tikvRPC` backoff class. Stream creation/publication errors now retire every selected direct/forwarded group independently before reporting the first error, matching `getClientAndSend`'s all-group attempt and preventing a later forwarding group from hanging behind a sibling failure. A real in-process Tonic stream test sends direct and forwarded batches through the generated `tikvpb.Tikv/BatchCommands` route, verifies correlated responses and source-compatible per-stream metadata on the selected common pool slot, then closes the first server stream and verifies a later request succeeds through its replacement. Close fails only unassigned queued entries; published IDs retain their separate response lifecycle. A malformed fewer-responses-than-IDs peer message is a safe Rust protocol error before partial retirement (rather than Go's unchecked-index panic). Remaining lifecycle requirements include the unavailable grpc-go connection-state monitor; Go's callback boundary maps natively through Rust futures. `legacy_codec.go` is not applicable: it only restores grpc-go's pre-1.66 borrowed protobuf-buffer ownership, whereas Prost messages own decoded bytes under Tonic's `ProstCodec`. `conn_monitor.go` requires a native connection-state source because public Tonic `Channel` does not expose grpc-go-equivalent connectivity states. Collapse is source-audited as a ResolveLock-only singleflight wrapper but awaits the same dynamic command/client ownership layer. Batch transport and the other `internal/client` components must still be transcreated as one complete package receipt. |
...
| `internal/locate` | `src/region.rs`, `src/region_cache.rs`, `src/pd`, `src/request`, `src/store` | seed | Atomic source inventory: production `accessmode.go` (62), `metrics_collector.go` (136), `pd_codec.go` (219), `region_cache.go` (3,472), `region_request.go` (2,449), `replica_selector.go` (744), `slow_score.go` (192), `sorted_btree.go` (151), and `store_cache.go` (1,222); support/tests `main_test.go` (32), `metrics_collector_test.go` (157), `pd_codec_test.go` (40), `region_cache_test.go` (3,497), `region_request3_test.go` (1,800), `region_request_state_test.go` (769), `region_request_test.go` (1,274), and `replica_selector_test.go` (3,916). No package-local generated/build/fixture/doc artifacts. Rust has only compatible seeds, including end-inclusive reverse lookup through PD `GetPrevRegion`, source exhausted-leader fallback to a follower leader-read probe, and partial sender/cache behavior. A real PdRpcClient transport failure now identifies the physical store independently of the logical peer, probes `grpc.health.v1.Health/Check` under the source `store-liveness-timeout`, records serving/unknown/unreachable status, and retains its region snapshot for selector-driven retry. Cached regions capture each peer store's failure epoch; an unhealthy physical store advances that epoch only if the request owns the captured snapshot, marks its health slow, and excludes stale replicas from retry selection. An unhealthy TiKV store owns one one-second health loop, which re-resolves its PD store metadata every 30 seconds while retaining the prior liveness state across refresh, and ends on recovery or client close. Configured `StoreLimit` now installs at `PdRpcClient` construction; it shares one deliberately optimistic atomic counter per logical region-route store, records client-go's address/store-labeled token-limit metric, and a Dispatch-lifetime guard releases a token after every transport result. The separate all-store route deliberately remains unlimited, as in client-go. Complete region/store cache lifecycle, PD codec, access/health/slow-score, replica/proxy selection, sender retry/error/cancellation/runtime metrics, and every source test remain required for one receipt. |
...
| `tikv` | client construction, PD/store integration | unassessed | High-level Go store package spans multiple Rust modules. |
...
| `txnkv/rangetask` | `src/transaction/range_task.rs`, `src/transaction/client.rs`, `src/transaction/requests.rs`, `src/stats.rs` | seed | Atomic source inventory: `range_task.go` (358 lines) and `delete_range.go` (185 lines); no package-local Go test, generated input, fixture, or build variant. Rust exposes caller-selected DeleteRange concurrency and `notify_only` execution, while the compatibility API defaults to one worker. Its runner covers client-go's bounded `ScanRegions`/cache batch-load producer loop (including legacy and extended PD response shapes, encoded V1/V2 cache keys, empty/gapped PD result retry, leaderless-result filtering/retry, and a fresh cancellation-owned 20,000-ms `BoPDRPC` backoffer for each load), default 128-region groups, a source-shaped `concurrency`-capacity producer/worker queue, bounded concurrent handlers, completed/failed counters, shared first-error cancellation, source `range_task_push_duration` observations around each enqueue attempt, source metric/log identifiers, redacted empty/start/progress/error/finish logging at the ten-minute default or caller-selected cadence, a fresh source `deleteRangeOneRegionMaxBackoff` (100,000-ms input, weighted by the same retry variables) for every sent region, source-exact terminal response-error handling, and client-go's completion/failure gauge/reset behavior. A full receipt still requires every source behavior and package-level validation. |
| `txnkv/transaction` | `src/transaction/transaction.rs`, requests, buffer | unassessed | Largest behavior package; complete only with differential transaction tests. |
| `txnkv/txnlock` | `src/transaction/lock.rs`, `src/request/plan.rs`, transaction request contexts | seed | Atomic inventory: production `lock.go` (43) and `lock_resolver.go` (1,724); tests/support `lock_test.go` (63), `lock_resolver_test.go` (160), `main_test.go` (25), and `test_probe.go` (129), with no generated/build/fixture artifacts. Rust resolves ordinary exclusive locks, caches only source-determined committed/rolled-back statuses in the source 2,048-entry FIFO (rejecting conflicting final outcomes), and exposes source resolving-lock record/update/done/snapshot state with stable slots. The generic retry wrapper observes snapshot, prewrite, and pessimistic lock lifetimes, including Rust future cancellation. Prewrite carries per-shard `TxnSize`; ordinary small-lock resolution follows the configured 512-default lite threshold. Read retries implement `ResolveLocksForRead` with per-transaction resolved/committed TiKV context hints. Transaction-client-created reads and writes retain the client-scoped resolver context through the `Committer` handoff, sharing final-status cache/observer state across all retry plans. Final ordinary read locks now enter a process-wide 10,000-permit detached cleanup path; eligible lite keys are collected per transaction and sent as exact per-region requests, recovered async-commit secondary/primary sets use the same path, and multi-key cleanup follows client-go's parent-plus-per-region bounded-pool fan-out. A saturated pool falls back inline without affecting the read, and NextGen non-lite requests set TiKV `is_async`. Detached cleanup exposes the source-named running-task gauges and saturated-pool fallback actions, including async-commit region tasks; an isolated-pool regression covers source admission, saturation fallback, release, and re-admission. The native resolver records source action counters for resolve entry, status classification (including expired/live and async-commit), CheckTxnStatus requests/final outcomes, ResolveLock (including lite), and BatchResolveLock. An internal resolve result now retains the source minimum remaining live-lock TTL and caps the legacy plan retry sleep, emitting `wait_expired`; full `BackoffWithMaxSleepTxnLockFast` adoption remains open. Cleanup suppresses empty batch ResolveLock requests and uses PessimisticRollback for pessimistic locks (including concrete `SharedPessimisticLock` holders); direct resolver input rejects only shared wrappers. Expired async-commit recovery performs source-shaped secondary status determination. Lock resolver write RPCs carry the 20,000-ms execution duration and resource-control/RU accounting. Rust still lacks client-go's `KVStore` owner, pipelined-flush observer ownership, per-shard resolver action metrics, owner shutdown, full TxnLockFast retry ownership, and full source-test coverage required for a receipt. |
| `txnkv/txnsnapshot` | `src/transaction/snapshot.rs`, `src/transaction/transaction.rs`, read request plans | seed | Atomic source inventory: production `client_helper.go` (168), `scan.go` (360), `snapshot.go` (1,414), and `snapshot_async.go` (301); test/support `test_probe.go` (74). There are no colocated Go test files, fixtures, generated inputs/outputs, package build files, or build-tag variants. Rust exposes a thin `Snapshot` facade over `Transaction` with initial Get/BatchGet/scan/reverse-scan, replica-read, stale-read, and retry-plan seeds. `set_snapshot_timestamp` now validates large versions, invalidates native cached reads, and clears only the source `resolvedLocks` hint set. Cache observability/content mutation and `ReturnCommitTs` reads now cover hits, size, copy, overwrite, targeted cleanup, cached misses, retained commit timestamps, the latest-timestamp no-cache guard, source `need_commit_ts` requests, zero-timestamp errors, and refetch eligibility. Completion requires runtime-statistics surface, async API and callback contract, scanner/range semantics, source lock-result ownership (including minimum-TTL retry), test-probe equivalents, and package-level differential validation. |
...
No real-cluster test applies to the package-owned deterministic defaults, validation, parsing, build selection, or RU arithmetic. Actual TLS handshakes, forwarding, batching, region refresh, transaction-file operation, and request-level RU collection remain on `internal/client`, `internal/locate`, `tikvrpc`, and transaction consumer rows. The host has no Go toolchain, so the original Go tests and goleak harness could not run locally; every original assertion executes in Rust.
...
Focused pinned-nightly tests cover a RawGet region-miss retry, deterministic per-shard fork/final-child merge accounting, and cancellation of a sibling already waiting in its cumulative retry. `cargo +nightly-2026-08-22 test -p tikv-client --lib --all-features --quiet` passes 302 tests, `cargo +nightly-2026-08-22 fmt --all -- --check` and `git diff --check` pass. This is not a `rawkv` completion receipt: full parity still depends on the separately incomplete source `internal/locate.RegionRequestSender` replica-selection and complete error-classification path, as well as the remaining raw package inventory and integration gates.
...
`txnkv/txnsnapshot` seed: runtime stats now retain retry class names/counts/scheduled sleeps across snapshot region retries and live-lock `txnLockFast` sleeps. The server-busy retry regression observes `tikvServerBusy`. This is observability over Rust's existing legacy retry schedule, not a claim that snapshot reads have adopted client-go's cumulative 20,000-ms backoffer; that behavioral conversion remains incomplete.
