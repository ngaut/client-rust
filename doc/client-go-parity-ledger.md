...
`internal/locate` sender seed: `StoreNotMatch` now also unconditionally retires the physical route address after cache invalidation, matching client-go's `CloseAddr(ctx.Addr)` even when PD metadata has already expired. `MismatchPeerId` remains a cache-only stop condition. Validation: focused `source_store_identity_errors_stop_the_current_send_loop` (1 passed), formatting, diff checks, and the full pinned-nightly library suite (368 passed) pass. The atomic source package receipt remains incomplete.
