...
| `internal/client` | `src/store`, `src/pd`, transport | in-progress | Complete source inventory recorded: production `client.go`, `client_async.go`, `client_batch.go`, `client_collapse.go`, `client_interceptor.go`, `conn_batch.go`, `conn_monitor.go`, `conn_pool.go`, `legacy_codec.go`, and `priority_queue.go`; support `mockserver/mock_tikv_service.go`; tests `client_async_test.go`, `client_batch_test.go`, `client_fail_test.go`, `client_interceptor_test.go`, `client_test.go`, `priority_queue_test.go`, and goleak `main_test.go`; `OWNERS` is repository metadata only. Rust maps source connection settings, versioned pool lifecycle, request/error identity, native-future cancellation, forwarding, priority/batch collection, direct/forwarded stream grouping, response correlation, health feedback, load adaptation, stream recreation, idle retirement, pending diagnostics, and batch request/stream metrics. ResolveLock requests with empty `keys` and `txn_infos` use one process-wide source-keyed singleflight `(region_id,start_version,is_async)`, a fixed 30-second physical timeout, independent caller timeouts, and cloned typed responses; lite and batch resolves bypass collapse. Native wrappers cover CoprocessorStream, BatchCoprocessor, and EstablishMPPConnection, eagerly read the first item, and retain per-receive timeout/close ownership. The dynamic transport routes Debug GetRegionProperties over the Debug service without forwarding metadata, handles BatchCommands Empty's batch/local-unary split, and applies the source API-v2 stream encode/decode matrix (including CopStream's deliberate unsupported-decode error). Successful unary and CopStream responses patch and accumulate RU-v2 counters with source read/write, bypass, non-TiKV, and first-stream-response boundaries. Every physical outcome records source command/store/stale/scope dimensions and source-specific count/sum; successful responses with time details record client-minus-server network latency with V2 precedence. Rust Prometheus has no Summary collector, so the source summary's count/sum (it configures no quantiles) uses a histogram with the same labels/name and additional buckets. `OpenTracingEnable` now reaches production connectors and enables a process-wide native metadata injector on unary, debug, and streaming gRPC calls; its default no-op matches an unconfigured global tracer. Tonic exposes no grpc-go `GetState`; Rust records the same five gauge labels from owned pool events and clears every label on pool removal instead of fabricating polling. `legacy_codec.go` and `client_async.go` map respectively to Prost-owned decode buffers and Rust futures rather than compatibility/callback façades. Source execution-detail tracing reconstructs the exact historical span tree and timing through an opt-in task-scoped sink on unary and batch transports, with request-plan capture for fan-out/retry tasks. This remains in progress pending closure of the remaining dynamic command/API-codec ownership shared with `tikvrpc` and `internal/apicodec` and explicit receipts for every original async/failure/leak test artifact. Proxy selection is correctly owned by `internal/locate`, not this transport receipt. |
...
| `txnkv/transaction` | `src/transaction/{transaction,client,requests,lowering,buffer,unionstore}.rs` | seed | Atomic inventory: production `2pc.go` (2,466), `batch_getter.go` (138), `binlog.go` (52), `cleanup.go` (117), `commit.go` (283), `pessimistic.go` (708), `pipelined_flush.go` (537), `prewrite.go` (666), `txn.go` (2,146), and `txn_file.go` (1,501); always-built support `test_probe.go` (478) and `test_util.go` (518); tests `2pc_test.go` (181), `batch_getter_test.go` (143), `txn_file_test.go` (1,477), and `txn_test.go` (355). There are no package-local non-Go fixtures, generated inputs, or build variants. Rust has substantial compatible transaction, 2PC, pessimistic, pipelined-flush, batch-get, cleanup/commit/prewrite, buffer, and request-lowering seeds, backed by the complete unionstore/latch packages, but the complete txn-file protocol/lifecycle, binlog surface, committer actions/state transitions, failpoints/probes, all source tests, and differential real-TiKV validation are not yet accounted for. The package also depends on incomplete `internal/locate`, `tikv`, `tikvrpc`, and lock/snapshot paths; no completion claim is made. |
