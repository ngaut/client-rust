...
| `txnkv/rangetask` | `src/transaction/range_task.rs`, `src/transaction/client.rs`, `src/transaction/requests.rs`, `src/stats.rs` | seed | Atomic source inventory: `range_task.go` (358 lines) and `delete_range.go` (185 lines); no package-local Go test, generated input, fixture, or build variant. Rust exposes caller-selected DeleteRange concurrency and `notify_only` execution, while the compatibility API defaults to one worker. Its runner covers ordered physical-region rediscovery, default 128-region groups, a source-shaped `concurrency`-capacity producer/worker queue, bounded concurrent handlers, completed/failed counters, shared first-error cancellation, source `range_task_push_duration` observations around each enqueue attempt, source metric/log identifiers, redacted empty/start/progress/error/finish logging at the ten-minute default or caller-selected cadence, a fresh source `deleteRangeOneRegionMaxBackoff` (100,000-ms input, weighted by the same retry variables) for every sent region, source-exact terminal response-error handling, and client-go's completion/failure gauge/reset behavior. A full receipt still requires exact batch-load/locate retry behavior and every source behavior. |
